...
int ultrasonic(int echoPin, int trigPin) // Clears the trigPin 
{
...
{ 
  ...
  if (newData == true) 
  {
    ...
    Serial.print(received.substring(6,10));
    if(instruction == "CONTF") goForward(); //Need to fix this one
    ...
    if(instruction == "AVOIL") avoidWallLeft(data.toInt());
    if(instruction == "AVOIR") avoidWallRight(data.toInt());
    newData = false;
  } 
}
